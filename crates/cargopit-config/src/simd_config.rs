...
impl SimdSim {
    ...
    pub fn name(&self) -> &str {
        self.get_str(keys::SIMD_FIELD_NAME)
    }

    pub fn display_name(&self) -> String {
        let name = self.name();
        if name.is_empty() {
            keys::SIMD_UNNAMED.to_string()
        } else {
            name.to_string()
        }
    }
    ...
    pub fn field_display(&self, key: &str) -> String {
        if key == keys::SIMD_FIELD_TELEMETRY {
        ...
        match value {
            ...
            _ => keys::SIMD_VALUE_COMPLEX.to_string(),
        }
    }

    pub fn telemetry_source(&self) -> &'static str {
        if let Some(canonical) =
            canonical_telemetry_source(self.get_str(keys::SIMD_FIELD_TELEMETRY))
        {
            return canonical;
        }
        match libconfig::group_get(&self.settings, keys::SIMD_FIELD_USEUDP).and_then(Value::as_bool)
        {
            Some(true) => keys::SIMD_TELEMETRY_UDP,
            Some(false) => keys::SIMD_TELEMETRY_SHM,
            None => keys::SIMD_TELEMETRY_AUTO,
        }
    }
    ...
    pub fn set_telemetry_source(&mut self, source: &'static str) {
        libconfig::group_set(
            &mut self.settings,
            keys::SIMD_FIELD_TELEMETRY,
            Value::String(source.to_string()),
        );
        libconfig::group_remove(&mut self.settings, keys::SIMD_FIELD_USEUDP);
    }
    ...
    pub fn game_id(&self) -> Option<u64> {
        let value = libconfig::group_get(&self.settings, keys::SIMD_FIELD_GAMEID)?;
...
impl SimdConfig {
    pub fn index_for_game_id(&self, game_id: u64) -> Option<usize> {
        if game_id == keys::SETTINGS_GAME_IDLE {
            return None;
        }
        self.sims
            .iter()
...
pub fn to_value(config: &SimdConfig) -> Value {
    ...
    let sims = config
        ...
        .collect();
    libconfig::group_set(&mut root, keys::KEY_SIMS, Value::List(sims));
    Value::Group(root)
}
...
pub fn table_columns(config: &SimdConfig) -> Vec<String> {
    let mut columns: Vec<String> = keys::SIMD_TABLE_COLUMNS
        .iter()
...
pub fn column_min_width(key: &str) -> u16 {
    match key {
        keys::SIMD_FIELD_NAME => keys::SIMD_COL_NAME_MIN,
        keys::SIMD_FIELD_GAMEID => keys::SIMD_COL_GAMEID,
        keys::SIMD_FIELD_LAUNCHEXE | keys::SIMD_FIELD_LIVEEXE => keys::SIMD_COL_EXE_MIN,
        keys::SIMD_FIELD_BRIDGEDELAY => keys::SIMD_COL_BRIDGE,
        keys::SIMD_FIELD_SIMAPI => keys::SIMD_COL_SIMAPI,
        keys::SIMD_FIELD_TELEMETRY => keys::SIMD_COL_TELEMETRY,
        _ => keys::SIMD_COL_EXTRA_MIN,
    }
}
...
pub fn column_window(widths: &[u16], selected: usize, budget: u16) -> (usize, usize) {
    ...
    while end < widths.len() {
        let extra = keys::SIMD_COL_SPACING + widths[end];
    ...
    while start > 0 {
        let extra = keys::SIMD_COL_SPACING + widths[start - 1];
        if used.saturating_add(extra) > budget {
            break;
        }
        used = used.saturating_add(extra);
...
fn cycle_telemetry_source(current: &str, delta: i32) -> &'static str {
    let index = keys::SIMD_TELEMETRY_SOURCES
        .iter()
        .position(|source| *source == current)
        .unwrap_or(0);
    let len = keys::SIMD_TELEMETRY_SOURCE_COUNT as i32;
    let next = (index as i32 + delta).rem_euclid(len) as usize;
    keys::SIMD_TELEMETRY_SOURCES[next]
}
...
"#,
        ...
        assert_eq!(
            &columns[..keys::SIMD_FIELD_COUNT],
            &keys::SIMD_TABLE_COLUMNS
        );
        assert!(columns.iter().any(|column| column == "custom"));
        assert_eq!(config.sims[0].field_display(keys::SIMD_FIELD_NAME), "Demo");
        assert_eq!(config.sims[0].field_display("custom"), "kept");
    }

    #[test]
    fn column_window_keeps_selected_visible() {
        let widths: Vec<u16> = keys::SIMD_TABLE_COLUMNS
            .iter()
...
"#,
        ...
        let name_width = column_content_width(&config, keys::SIMD_FIELD_NAME);
    ...
    fn telemetry_defaults_to_auto_and_migrates_useudp() {
        ...
        assert_eq!(auto.sims[0].telemetry_source(), keys::SIMD_TELEMETRY_AUTO);
        assert_eq!(
            auto.sims[0].field_display(keys::SIMD_FIELD_TELEMETRY),
            keys::SIMD_TELEMETRY_AUTO
        );
        let shm = parse(r#"sims = ( { name = "A"; gameid = 1; useudp = false; } );"#).unwrap();
        assert_eq!(shm.sims[0].telemetry_source(), keys::SIMD_TELEMETRY_SHM);
        let udp = parse(r#"sims = ( { name = "A"; gameid = 1; useudp = true; } );"#).unwrap();
        assert_eq!(udp.sims[0].telemetry_source(), keys::SIMD_TELEMETRY_UDP);
        let named = parse(r#"sims = ( { name = "A"; gameid = 1; telemetry = "udp"; } );"#).unwrap();
        assert_eq!(named.sims[0].telemetry_source(), keys::SIMD_TELEMETRY_UDP);
    }
    ...
    fn telemetry_cycle_writes_source_and_drops_useudp() {
        ...
        assert_eq!(
            ...
            keys::SIMD_TELEMETRY_UDP
        );
        assert_eq!(
            config.sims[0].get_str(keys::SIMD_FIELD_TELEMETRY),
            keys::SIMD_TELEMETRY_UDP
        );
        assert!(libconfig::group_get(&config.sims[0].settings, keys::SIMD_FIELD_USEUDP).is_none());
        assert_eq!(
            config.sims[0].cycle_telemetry_source(1),
            keys::SIMD_TELEMETRY_AUTO
        );
...
"#,
        ...
        .unwrap();
        assert_eq!(config.index_for_game_id(keys::SETTINGS_GAME_IDLE), None);
