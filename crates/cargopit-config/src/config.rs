...
impl DeviceClass {
    pub fn as_str(self) -> &'static str {
        match self {
            DeviceClass::Usb => crate::keys::CLASS_USB,
            DeviceClass::Sound => crate::keys::CLASS_SOUND,
            DeviceClass::Serial => crate::keys::CLASS_SERIAL,
        }
    }
    ...
    pub fn all() -> [DeviceClass; crate::keys::DEVICE_CLASS_COUNT] {
...
impl Default for SimProfile {
    fn default() -> Self {
        Self {
            name: String::new(),
            sim: keys::DEFAULT_SIM.to_string(),
            car: keys::DEFAULT_CAR.to_string(),
            api: None,
            devices: Vec::new(),
            extra: Vec::new(),
        }
...
impl DeviceEntry {
    ...
    pub fn class(&self) -> DeviceClass {
        match self.get_str(keys::KEY_DEVICE) {
            Some(keys::CLASS_USB) => DeviceClass::Usb,
            Some(keys::CLASS_SERIAL) => DeviceClass::Serial,
            _ => DeviceClass::Sound,
        }
    }

    pub fn type_name(&self) -> &str {
        self.get_str(keys::KEY_TYPE).unwrap_or(keys::TYPE_HAPTIC)
    }

    pub fn enabled(&self) -> bool {
        self.get_bool(keys::KEY_ENABLED)
            .unwrap_or(keys::DEFAULT_ENABLED)
    }

    pub fn identity(&self) -> String {
        self.get_str(keys::KEY_DEVID)
            .or_else(|| self.get_str(keys::KEY_DEVPATH))
            .unwrap_or("")
            .to_string()
    }
    ...
    pub fn summary(&self) -> String {
        let class = self
            ...
            .unwrap_or(keys::SUMMARY_UNKNOWN_CLASS);
        let kind = self
            .get_str(keys::KEY_TYPE)
            .unwrap_or(keys::SUMMARY_UNKNOWN_TYPE);
        let extra = self
            .get_str(keys::KEY_SUBTYPE)
            .or_else(|| self.get_str(keys::KEY_EFFECT))
            .unwrap_or("");
        if extra.is_empty() {
            format!("{class} - {kind}")
        } else {
    ...
    pub fn keep_keys_for_class(&mut self, class: DeviceClass, type_name: &str, allowed: &[&str]) {
        self.settings
            .retain(|(key, _)| allowed.contains(&key.as_str()));
        self.set_str(keys::KEY_DEVICE, class.as_str());
        if class == DeviceClass::Sound && type_name == keys::TYPE_HAPTIC {
            self.remove(keys::KEY_TYPE);
        } else {
            self.set_str(keys::KEY_TYPE, type_name);
        }
    }
}
...
fn profile_from_value(value: &Value) -> Result<SimProfile> {
    ...
    for (key, item) in group {
        match key.as_str() {
            ...
            keys::KEY_SIM => {
                profile.sim = item.as_str().unwrap_or(keys::DEFAULT_SIM).to_string();
            }
            keys::KEY_CAR => {
                profile.car = item.as_str().unwrap_or(keys::DEFAULT_CAR).to_string();
            }
            keys::KEY_API => {
...
pub fn to_value(config: &CargopitConfig) -> Value {
    ...
    let profiles = config
        ...
        .collect::<Vec<_>>();
    libconfig::group_set(&mut root, keys::KEY_CONFIGS, Value::List(profiles));
    Value::Group(root)
}
...
fn profile_to_value(profile: &SimProfile) -> Value {
    ...
    if profile.name.is_empty() {
        libconfig::group_remove(&mut items, keys::KEY_PROFILE_NAME);
    } else {
        libconfig::group_set(
            &mut items,
            keys::KEY_PROFILE_NAME,
    ...
    if let Some(api) = &profile.api {
        libconfig::group_set(&mut items, keys::KEY_API, Value::String(api.clone()));
    } else {
        libconfig::group_remove(&mut items, keys::KEY_API);
    }
    let devices = profile
        .devices
        .iter()
        ...
        .collect();
    libconfig::group_set(&mut items, keys::KEY_DEVICES, Value::List(devices));
    Value::Group(items)
}
...
mod tests {
    ...
    fn parse_sample_config() {
        ...
        let usb = config.profiles[0]
            ...
            .expect("usb device");
        assert_eq!(usb.get_str(keys::KEY_TYPE), Some(keys::TYPE_TACHOMETER));
    }
    ...
    fn round_trip_catalog_keys() {
        ...
        device.set_str(keys::KEY_TYPE, keys::TYPE_SIMLEDS);
        device.set_str(keys::KEY_DEVPATH, "/dev/simdev0");
        device.set_int(keys::KEY_BAUD, 115200);
        device.set_float(keys::KEY_AMPFACTOR, 1.0);
        device.set_int(keys::KEY_NUMLEDS, 6);
        device.set_int(keys::KEY_STARTLED, 2);
        device.set_int(keys::KEY_ENDLED, 5);
        device.set_str(keys::KEY_CONFIG, "basic_rpms.lua");
        ...
        assert_eq!(again.get_i64(keys::KEY_NUMLEDS), Some(6));
        assert_eq!(again.get_f64(keys::KEY_AMPFACTOR), Some(1.0));
        assert_eq!(again.get_str(keys::KEY_DEVPATH), Some("/dev/simdev0"));
    }
    ...
    fn serial_wheel_keep_keys_preserves_moza_r9() {
        ...
        device.set_str(keys::KEY_DEVPATH, "/dev/serial/by-id/r9");
        device.set_int(keys::KEY_BAUD, 115200);
        device.keep_keys_for_class(
            DeviceClass::Serial,
            crate::keys::TYPE_WHEEL,
            &[
                crate::keys::KEY_DEVICE,
                crate::keys::KEY_TYPE,
                crate::keys::KEY_SUBTYPE,
                crate::keys::KEY_DEVPATH,
                crate::keys::KEY_BAUD,
            ],
        );
        assert_eq!(
            device.get_str(keys::KEY_SUBTYPE),
            Some(keys::SUBTYPE_MOZA_R9)
        );
        assert_eq!(
            device.get_str(keys::KEY_DEVPATH),
