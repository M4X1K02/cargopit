    ...
    if (serialdevice->port)
    {
        if (simData->playerflag != last_flag) {
            ...
            if (simData->playerflag == SIMAPI_FLAG_YELLOW) {
                ...
                cargopit_serial_write(serialdevice->id, fl, MOZA_COLOR_PAYLOAD_SIZE, MOZA_TIMEOUT);
                cargopit_serial_write(serialdevice->id, fr, MOZA_COLOR_PAYLOAD_SIZE, MOZA_TIMEOUT);
            } else {
        ...
        result = cargopit_serial_write(serialdevice->id, bytes, size, MOZA_TIMEOUT);
    }
...
int moza_ks_pro_wheel_init(SerialDevice* serialdevice, const char* portdev)
{
    ...
    for (size_t i = 0; i < payload_count; i++) {
        int result = cargopit_serial_write(serialdevice->id, payloads[i], MOZA_COLOR_PAYLOAD_SIZE, MOZA_TIMEOUT);
        if (result < 0) return result;
    }

    unsigned char p6[] = MOZA_FLAG_COLOR_OFF_RIGHT;
    p6[MOZA_COLOR_PAYLOAD_SIZE-1] = moza_checksum(p6, MOZA_COLOR_PAYLOAD_SIZE);
    int result = cargopit_serial_write(serialdevice->id, p6, MOZA_COLOR_PAYLOAD_SIZE, MOZA_TIMEOUT);
    if (result < 0) return result;
