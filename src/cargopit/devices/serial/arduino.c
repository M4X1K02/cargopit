...
int arduino_update(SerialDevice* serialdevice, void* data, size_t size)
{
    ...
    result = cargopit_serial_write(serialdevice->id, data, size, arduino_timeout);
...

{
    ...
    while (count < 4)
    {
        ...
        cargopit_wait_for_event(serialdevice->id, 6);

        result = cargopit_serial_write(serialdevice->id, &bytes1, bufsize1, arduino_timeout);


        cargopit_wait_for_event(serialdevice->id, 5);
        bytesWaiting = cargopit_input_wait(serialdevice->id);
        if (bytesWaiting > 0)
        {
            memset(buf, 0, sizeof(buf));
            retval = cargopit_serial_read_block(serialdevice->id, buf, sizeof(buf)-1, 10);
            if (retval < 0)
            {
                retval = -1;
                break;
...
int arduino_simled_update(SerialDevice* serialdevice, SimData* simdata)
{
    ...
    result = cargopit_serial_write(serialdevice->id, &bytes, size, arduino_timeout);
...
int arduino_customled_free(SerialDevice* serialdevice, bool lua)
{
    ...
    int result = cargopit_serial_write_block(serialdevice->id, &bytes, size, arduino_timeout);
