...
- Streams are labelled per effect and tyre (`cargopit.TyreSlip.FrontLeft`) so any PipeWire processor (Easy Effects, Carla, filter-chain) can do EQ, limiting, and rig correction. A filter-chain preset and a generator for your own seat sweep are in `conf/pipewire/` and `tools/haptics/`. See [HOW-TO-USE](HOW-TO-USE.md#external-processing-eq-limiter-rig-correction).
